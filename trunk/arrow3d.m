function varargout=arrow3d(start,stop,color1,color2,ang,ltype,p,n)
%   Plot 3d Arrows
% 
...
%Set default data
if nargin<3 color1=[1 0 0]; end
if nargin<4 color2=[1 0 0]; end
if nargin<5 ang=15; end
if nargin<6 ltype='cylinder'; end
if nargin<7 p=[0.25,0.10]; end
if nargin<8 n=[20,10]; end
...
if strcmp(ltype,'line')
    ...
    for i=1:length(dis)
        ...
        hlines(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',color2,'facealpha',0.5);
        hlgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),color2);
    end
end
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(cx,cy,cz,'edgecolor', [0.8 0.8 0.8],'facecolor',color1,'facealpha',1.0);
    %Draw the underside of taper
    hhgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),color1);
end
if SET_HOLD_OFF
    hold off;
end
